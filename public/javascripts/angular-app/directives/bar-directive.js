    ...
    function(d3, $rootScope, myConfig) {
        ...
        return {
            ...
            compile: function(elements, attrs, transclude) {
                var svg = d3.select(elements[0]).append('svg').attr('width', "100%").attr('height', "10");
                // var entry = d3.select(elements[0]).append("div")
                // 	.attr("class", "text-center")
                // 	.style("color", "#2980B9")
                // 	.html("<i class='fa fa-arrow-up fa-3x' aria-hidden='true' style='color: #2980B9'></i>");
                 
                svg.append('g').attr("class","background").append('rect').attr('height', 10).attr('width', "100%").style('fill', "#303030");
